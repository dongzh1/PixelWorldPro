...
object Structure {
    ...
    fun setStruction(world: World, structure: StructureCreateData){
        for (xyz in structure.block.keys){
            ...
            if (material == null){
                Bukkit.getConsoleSender().sendMessage("§aPixelWorldPro 无法从Bukkit内找到名为 ${structure.block[xyz]} 的方块")
                Bukkit.getConsoleSender().sendMessage("§aPixelWorldPro 这不是一个PixelWorldPro的BUG，如果你不知道如何解决它，您可以询问管理员或者发布悬赏贴")
                continue
            }
            block.type = material
        }
    ...
    private fun buildStructureCreateData(buildX:Int, buildY: Int, buildZ: Int, name: String): StructureCreateData?{
        ...
        if (config == null){
            Bukkit.getConsoleSender().sendMessage("§aPixelWorldPro 读取结构文件 $name 失败，请检查文件路径或者文件是否存在。Linux等请检查是否有读写权限")
            return null
        }
        val showName = config.getString("showName")
        if (showName == null){
            Bukkit.getConsoleSender().sendMessage("§aPixelWorldPro ${config.name}不是一个有效的结构文件，因为 showName 为空")
            return null
        }
        val realName = config.getString("realName")
        if (realName == null){
            Bukkit.getConsoleSender().sendMessage("§aPixelWorldPro ${config.name}不是一个有效的结构文件，因为 realName 为空")
            return null
        }
        val blockList = config.getConfigurationSection("xy")!!.getKeys(false)
        if (blockList.isEmpty()){
            Bukkit.getConsoleSender().sendMessage("§aPixelWorldPro ${config.name}不是一个有效的结构文件，因为 xy 列表没有内容")
            //return null
        }
        var z = buildZ
        ...
        for (xy in blockList) {
            ...
            for (buildBlock in buildBlockList) {
                for (block in buildBlock.split("")) {
                    ...
                    if (blockName == null){
                        Bukkit.getConsoleSender().sendMessage("§aPixelWorldPro 无法从${config.name}中寻找到方块数据 $block")
                        return null
                    }
    ...
    fun playerJoin(isJump: Boolean, location: Location, world: World, player: Player){
        ...
        if (structure == null){
            Bukkit.getConsoleSender().sendMessage("§aPixelWorldPro 无法在Dimension.yml中找到地狱门配置[Structure.netherFile选项]")
            return
        }
